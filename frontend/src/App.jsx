...
export default function App() {
  ...
  // Update volume slider background based on volume value
  const updateVolumeSliderBackground = useCallback((volumeValue) => {
    const percentage = Math.round(volumeValue * 100);
    const slider = document.querySelector('.volume-slider');
    if (slider) {
      // Update the background gradient to show the current volume level
      slider.style.backgroundImage = `linear-gradient(to right, #ffffff 0%, #ffffff ${percentage}%, transparent ${percentage}%, transparent 100%)`;
    }
  }, []);
  ...
  }, [volume, updateVolumeSliderBackground]);

  // Handle volume slider hover effects
  useEffect(() => {
    const slider = document.querySelector('.volume-slider');
    if (!slider) return;
    ...
    const handleMouseEnter = () => {
      ...
      slider.style.backgroundImage = `linear-gradient(to right, #ffffff 0%, #ffffff ${percentage}%, rgba(255, 255, 255, 0.2) ${percentage}%, rgba(255, 255, 255, 0.2) 100%)`;
    };
    ...
    <div className="app">
      <header>
        ...
        <div className="volume-controls">
          {/* Volume Icon */}
          <button className="volume-icon-btn" onClick={() => {
